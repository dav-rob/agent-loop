...
- Planner verification is declarative: tasks carry `verification_requirements`, while executors establish their environment and demonstrate the work and reviewers independently rerun appropriate checks. The orchestrator must never execute a planner/model/config/database string as a project command.
...
- Legacy planner verification commands are inert compatibility data. On resume, repeated legacy exit-127 verification failures may be recovered once so the task can proceed through the agent-owned verification lifecycle without discarding prior attempts.
